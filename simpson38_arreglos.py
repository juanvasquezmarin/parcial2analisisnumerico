def simpson38_arreglos(x, y):
    """
    Implementación del método de Simpson 3/8 para datos discretos.
    
    Parámetros:
    x : arreglo de valores de x (debe tener espaciamiento constante)
    y : arreglo de valores de y correspondientes a x
    """
    n = len(x) - 1
    if n % 3 != 0:
        raise ValueError("El número de intervalos debe ser múltiplo de 3 para Simpson 3/8")
    
    # Calculamos h asumiendo espaciamiento constante
    h = x[1] - x[0]
    
    suma = y[0] + y[-1]
    
    for i in range(1, n):
        if i % 3 == 0:
            suma += 2 * y[i]
        else:
            suma += 3 * y[i]
            
    return (3 * h / 8) * suma

# Ejemplo de uso
if __name__ == "__main__":
    # Datos para y = x^2 en [0, 1] con n=3 (4 puntos)
    x_datos = [0, 7, 14, 21, 28, 35, 42, 49, 56, 63, 70, 77, 84]
    y_datos = [0.02, 0.077, 0.206, 0.431, 0.766, 1.163, 1.559, 1.907, 2.18, 2.377, 2.511, 2.604, 2.67] # y = x^2
    
    
    resultado = simpson38_arreglos(x_datos, y_datos)
    print(f"Resultado Simpson 3/8 (arreglos): {resultado}")
